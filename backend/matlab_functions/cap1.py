...
def calcular_biseccion(f_str, a, b, niter, tol):
    ...
    iter, ai, xm, bi, fm, err, mes, mes_err = eng.biseccion(f, float(a), float(b), int(niter), float(tol), nargout=8)
    
    response = formatter.biseccion_formatter(iter, ai, xm, bi, fm, err, mes, mes_err)
    return response

def calcular_newton(f_str, x0, niter, tol):
    f = eval_function(eng, f_str)
...
def calcular_regla_falsa(f_str, a, b, niter, tol):
    f = eval_function(eng, f_str)
...
def calcular_secante(f_str, x0, x1, niter, tol):
    f = eval_function(eng, f_str)
    n, xn, fm, err, mes, mes_err = eng.secante(f, matlab.double(x0), matlab.double(x1), matlab.double(tol), matlab.double(niter), nargout=6)
