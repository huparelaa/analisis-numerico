...
const Newton = ({ name }) => {
  ...
  const [dif_divididas,setMatriz] = useState([[]])
  ...
  const handleSubmit = async event => {
    ...
    try{
        ...
        const res = await fetch("http://127.0.0.1:8000/api/interpolation/calcular_newton/", 
        {
            method: "POST", 
            mode: "cors", 
            cache: "no-cache",
            ...
            data => {
                ...
                setMatriz(data["Tabla_diferencias_divididas"])
                const coef = data["coeficientes_del_polinomio"]
                console.log("pol: ",pol)
                console.log("dif_divididas: ",dif_divididas)
  ...
  return (
    ...
    <p>
        <li>
        <strong>Polinomio de Newton: </strong>
        {pol}
        </li>
        
    </p>
    <React.Fragment>
              <strong> <p>Tabla de diferencias dividias</p> </strong>
              <BlockMath
                math={
                  renderLatexMatrix(dif_divididas) 
                }
              />
                      <LinkGraph>
                <a
