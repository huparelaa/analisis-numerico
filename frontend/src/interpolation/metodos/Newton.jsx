...
const Newton = ({ name }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      try {
        const response = await axios.post(`${url}/interpolation/newton`, data, {
          headers: {
        ...
        setPol(responseData["polinomio"]);
        setMatriz(responseData["tabla"]);
      } catch (e) {
        setError(e.toString());
      }
    } catch (e) {
      ...
      {data && !error ? (
        <Results>
          ...
          <React.Fragment>
            <strong>
              {" "}
              <p>Tabla de diferencias dividias</p>{" "}
            </strong>
            <BlockMath math={renderLatexMatrix(dif_divididas)} />
            <LinkGraph>
              <a
                href={
                  "/graficar?function=" +
