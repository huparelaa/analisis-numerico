...
const Vandermonde = ({ name }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      try {
        const response = await axios.post(`${url}/interpolation/vandermonde`, data, {
          headers: {
      ...
      {data && !error ? (
        <Results>
          ...
          <React.Fragment>
            <strong>
              {" "}
              <p>Matriz de Vandermonde</p>{" "}
            </strong>
            <BlockMath math={renderLatexMatrix(matriz)} />
            <LinkGraph>
              <a
                href={
                  "/graficar?function=" +
